...
const getStockInfoCommand = new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('company_name').setDescription('Company name to get stock info?').setRequired(true))
...
const subscribeCommand = new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('company_name').setDescription('Company name to subscribe stock info?').setRequired(true))
...
const commands = [
    getStockInfoCommand.toJSON(),
    subscribeCommand.toJSON(),
    unsubCommand.toJSON(),
];

const rest = new REST({ version: "9" }).setToken(
    "NzY4OTMzNTkxOTcxMjY2NTgw.G4CVEL.voyuEKoI30wVf-uC4sI20OTIswzvl-HaFy9JKY"
);
